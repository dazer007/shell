...
#启动基础模块
base(){
	docker-compose up -d  sentinel seata-server web-nginx blade-nginx
}
...
#启动程序模块
modules(){
	docker-compose up -d blade-gateway1 blade-gateway2  blade-auth1 blade-auth2  blade-user  blade-desk blade-system blade-log  blade-resource blade-content
}
